...
def run(args, device, logger):
    ...
    for stage, epochs in enumerate(args.stages):
        if stage > 0 and args.use_rlu:
            ...
            logger.info("This history model Train ACC is {}".format(evaluator(labels[:train_node_nums],predict_prob[:train_node_nums].argmax(dim=-1, keepdim=True).cpu())))

            logger.info("This history model Valid ACC is {}".format(evaluator(labels[train_node_nums:train_node_nums+valid_node_nums],predict_prob[train_node_nums:train_node_nums+valid_node_nums].argmax(dim=-1, keepdim=True).cpu())))

            logger.info("This history model Test ACC is {}".format(evaluator(labels[train_node_nums+valid_node_nums:train_node_nums+valid_node_nums+test_node_nums],predict_prob[train_node_nums+valid_node_nums:train_node_nums+valid_node_nums+test_node_nums].argmax(dim=-1, keepdim=True).cpu())))
        ...
        if args.giant and args.method == "SAGN":
            ...
            logger.info("Stage: {}, Best Epoch {}, Val {:.4f}, Test {:.4f}".format(
            ...
            for epoch in range(epochs):
                ...
                logger.info(log)    
            logger.info("Best Epoch {}, Val {:.4f}, Test {:.4f}".format(
                best_epoch, best_val, best_test))
...
def main(args):
    ...
    val_accs = []
    test_accs = []
    logger = setup_logger("logs", 0)
    for i in range(args.num_runs):
        logger.info(f"Run {i} start training")
        set_seed(args.seed+i)
        best_val, best_test = run(args, device, logger)
        #np.save(f"output/{args.dataset}/output_{i}.npy", preds.numpy())
        val_accs.append(best_val)
        test_accs.append(best_test)

    logger.info(f"Average val accuracy: {np.mean(val_accs):.4f}, "
